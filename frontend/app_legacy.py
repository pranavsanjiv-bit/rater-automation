...
# ── Section: dimension pool + role assignment ─────────────────────────────────
...
def render_dims_section():
    ...
    with hdr_col:
        st.markdown("#### Dimensions (Rate Table)")
    ...
    if dims:
        lh1, lh2, lh3, lh4 = st.columns([3, 1.5, 3, 0.7])
        with lh1:
            st.caption("Name")
        with lh2:
            st.caption("Type")
    ...
    for i, dim in enumerate(dims):
        with st.container(border=True):
            c_name, c_type, c_role, c_del = st.columns([3, 1.5, 3, 0.7])
            ...
            with c_role:
                if role == "row":
                    ...
                    with rc1:
                        st.markdown(
                            '<span class="pill pill-green">● Row</span>',
                            unsafe_allow_html=True,
                        )
                    with rc2:
                    ...
                    with rc1:
                        st.markdown(
                            '<span class="pill pill-blue">● Column</span>',
                            unsafe_allow_html=True,
                        )
                    with rc2:
    ...
    if not dims:
        st.info("No dimensions yet. Use the button above to add one.", icon="ℹ️")
    ...
    if st.session_state.form_open:
        with st.container(border=True):
            ...
            render_dim_form("dim", st.session_state.form, "do_confirm", "do_cancel")

        if st.session_state.get("do_confirm"):
            st.session_state.do_confirm = False
            f = st.session_state.form
            if f["name"].strip():
                st.session_state.dims.append(build_dim_from_form(f))
...
def _get_dim_slug_options() -> list[str]:
    """Return all outer-dimension slugs available as formula variables."""
    slugs = []
    seen = set()
    row_dim = _row_dim()
    col_dim = _col_dim()
    for dim in st.session_state.dims:
        ...
        seen.add(slug)
        slugs.append(slug)
    # Also expose row/col axis slugs if set
    if row_dim:
...
def render_tab_template_builder():
    ...
    # ── Top panel: Dimensions + Formula Variables side-by-side ───────────────
    dim_col, fv_col = st.columns(2)
    with dim_col:
        render_dims_section()
    with fv_col:
...
def _render_calc_inputs(full_def: dict) -> dict:
    ...
    for fv in calculation.get("formula_variables", []):
        ...
        try:
            default_num = float(fv_default) if fv_default not in (None, "") else 0.0
        except ValueError:
            default_num = 0.0
        val = st.number_input(
            fv["name"],
    ...
    if st.button("Calculate", type="primary", key="do_calculate"):
        try:
            ...
            if resp.ok:
                ...
                with res_col1:
                    st.metric("Rater value", f"{data['rater_val']:,.4f}")
                with res_col2:
                    st.metric("Base premium", f"{data['base_premium']:,.4f}")
                with res_col3:
                    st.metric("Total premium", f"{(data['base_premium'] * 1.18):,.4f}")

                with st.expander("Resolved buckets (debug)"):
                    st.json(data.get("resolved_buckets", {}))
                with st.expander("Lookup key"):
                    st.code(data.get("lookup_key", ""))
